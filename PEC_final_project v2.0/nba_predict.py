...
k = 3
def preprocess_data():
    data_1617 = pd.read_csv('PEC_final_project v2.0/nba_player_1617.csv')
    data_1617.insert(0,'SEASON','1617')
    data_1718 = pd.read_csv('PEC_final_project v2.0/nba_player_1718.csv')
    data_1718.insert(0,'SEASON','1718')
    data_1819 = pd.read_csv('PEC_final_project v2.0/nba_player_1819.csv')
...
def team_analysis(data):
    ...
    for index, item in enumerate(data):
        for h in range(k):
            if item[8] == h:
                ...
                continue
    team = []
    team_winning_rate = pd.read_csv('PEC_final_project v2.0/team_winning_rate.csv')
    ...
    df2.to_csv('PEC_final_project v2.0/team_analysis.csv',index=True)
...
def main():
    ...
    with open('PEC_final_project v2.0/kmeans.p', 'rb') as outfile:
    ...
    fig = plt.figure()
    ax = Axes3D(fig)
    ax.scatter([x[0] for x in data],[x[1] for x in data],[x[2] for x in data],c=labels)
    fig = plt.figure()
    ax = Axes3D(fig)
    ...
    try_different_method(model_DecisionTreeRegressor,x_train,x_test,y_train,y_test, 'DecisionTreeRegressor')
    try_different_method(model_SVR,x_train,x_test,y_train,y_test, 'SVR')
    try_different_method(model_RandomForestRegressor,x_train,x_test,y_train,y_test, 'RandomForestRegressor')
