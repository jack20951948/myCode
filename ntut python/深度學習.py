...
def test33():    
    x_data = np.random.rand(50,2)  #隨機0~1的值
...
def test06():
    ...
    d51 = tf.sin(d50)
    A=sess.run(d51, feed_dict={d50: 0.2})   #feed_dict是關鍵字，必須寫成這個名字，後面緊跟傳入的參數
    print(A)
...
def test00():
    ...
    x_data = np.random.rand(100).astype(np.float32) #astype可以轉換陣列裡面的資料型態
